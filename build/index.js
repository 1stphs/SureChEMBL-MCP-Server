...
const isValidDocumentArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
...
const isValidStructureArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
...
const isValidExportArgs = (args) => {
    return (typeof args === 'object' &&
        ...
        args.chemical_ids.every((id) => typeof id === 'string' && id.length > 0) &&
...
class SureChEMBLServer {
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            if (documentMatch) {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to fetch document ${docId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
            ...
            if (chemicalMatch) {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to fetch chemical ${chemId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
            ...
            if (familyMatch) {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to fetch patent family ${patentId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
            ...
            if (searchMatch) {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to search chemicals: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
    ...
    async handleSearchPatents(args) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to search patents: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async handleGetDocumentContent(args) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get document content: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async handleGetPatentFamily(args) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get patent family: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async handleSearchByPatentNumber(args) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to find patent: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    // Chemical Search & Retrieval handlers
    async handleSearchChemicalsByName(args) {
    ...
    async handleGetChemicalById(args) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get chemical: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async handleSearchByInchi(args) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to search by InChI: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    // Structure & Visualization handlers
    async handleGetChemicalImage(args) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to generate chemical image: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async handleGetChemicalProperties(args) {
        if (!args || typeof args.chemical_id !== 'string') {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid chemical ID');
        }
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get chemical properties: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    // Data Export & Analysis handlers
    async handleExportChemicals(args) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to export chemicals: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async handleAnalyzePatentChemistry(args) {
        if (!isValidDocumentArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid document arguments');
        }
        try {
            ...
            const chemicalAnnotations = [];
            // Process abstracts
            if (document.contents?.patentDocument?.abstracts) {
                document.contents.patentDocument.abstracts.forEach((abstract) => {
                    if (abstract.section?.annotations) {
                        abstract.section.annotations.forEach((annotation) => {
            ...
            if (document.contents?.patentDocument?.descriptions) {
                document.contents.patentDocument.descriptions.forEach((description) => {
                    if (description.section?.annotations) {
                        description.section.annotations.forEach((annotation) => {
                            chemicalAnnotations.push({
                                source: 'description',
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to analyze patent chemistry: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    // Advanced Analysis Tools handlers
    async handleGetChemicalFrequency(args) {
        if (!args || typeof args.chemical_id !== 'string') {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid chemical ID');
        }
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get chemical frequency: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async handleSearchSimilarStructures(args) {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to search similar structures: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    async handleGetPatentStatistics(args) {
        if (!isValidDocumentArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid document arguments');
        }
        try {
            ...
            const chemicalFrequencies = chemicalAnnotations.reduce((acc, annotation) => {
            ...
            const statistics = {
                document_id: args.document_id,
                document_info: {
                    title: docInfo?.inventionTitles?.find((t) => t.lang === 'EN')?.title || 'N/A',
                ...
                content_statistics: {
                    ...
                    languages: [...new Set([...abstracts, ...descriptions].map((s) => s.lang))]
                },
                chemical_statistics: {
                    total_chemical_annotations: chemicalAnnotations.length,
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get patent statistics: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    // Helper methods
    ...
    calculateRarityScore(frequency) {
        if (frequency === 0)
            return 0;
        if (frequency === 1)
            return 1.0;
